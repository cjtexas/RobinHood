...
api_orders <- function (RH, action, status_url = NULL, cancel_url = NULL, instrument_id = NULL,
  ...
  if (action == "order") {
    ...
    if(dta$status_code!=201){
      stop(content(dta)$non_field_errors[[1]][1])
    }
  ...
  if (action == "history") {
    ...
    url <- paste(RobinHood::api_endpoints("orders"), "?account_numbers=", RH$url.account_number,
                 "&page_size=", page_size, '&', sep = "")
    token <- paste("Bearer", RH$api_response.access_token)
