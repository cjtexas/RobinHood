...
#' @param notional (number) dollar amount to invest
#' @param currency (string) currency to use for notional
...
#' place_order(RH = RH,
#'             symbol = "GE",          # Ticker symbol you want to trade
#'             notional = 5,           # Dollar amount of trade
#'             side = "buy")  
#'
#' # Stop loss example
#' place_order(RH,
...
#'             side = 'sell')
#'}
place_order <- function(RH, symbol, type = 'market', time_in_force = 'gfd', trigger = 'immediate', 
                        price = NA, stop_price = NA,
                        quantity = 0, notional = NA, currency = 'USD', side) {
